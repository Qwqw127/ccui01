...
import {DNotificationService} from '../../notification';
    ...
    setup(props: UploadProps, ctx) {
        ...
        const checkValid = () => {
            ...
            if (oneTimeUpload.value) {
                const checkResult = checkAllFilesSize(
                    totalFileSize,
                    uploadOptions?.value?.maximumSize || 0
                );
                if (checkResult && checkResult.checkError) {
                    removeFiles();
                    alertMsg(checkResult.errorMsg);
        ...
        const canUpload = () => {
            ...
            if (beforeUpload?.value) {
                const result: boolean | Promise<boolean> = beforeUpload?.value(
                    getFullFiles()
                );
        ...
        const fileUpload = (event?: Event, fileUploader?: FileUploader) => {
            ...
            canUpload().then((_canUpload) => {
                ...
                if (typeof httpRequest?.value === 'function') {
                    const files = fileUploaders.value.map(
                        (tempFileUploader) => tempFileUploader.file
                    );
                    httpRequest.value(files);

                    return;
                }
                const uploadObservable: Promise<IFileResponse[]> = oneTimeUpload.value
                    ? _oneTimeUpload()
                    : upload(fileUploader);
                props.onProgress &&
                props.onProgress(selectedFiles.value, modelValue.value);
                uploadObservable
        ...
        const _dealFiles = (promise: Promise<File[]>) => {
            ...
            promise
                .then((files) => {
                    if (
                        limit.value &&
                        modelValue.value.length + files.length > limit.value
                    ) {
                    ...
                    if (
                        uploadOptions?.value &&
                        uploadOptions.value.checkSameName &&
                        sameNameFiles.length
                    ) {
        ...
        return () => (
            <div>
                ...
                >
                    {ctx.slots.default?.() ? (
                        ...
                        >
                            {fileUploaders.value.length === 0 && (
                                <div class={[formControlNs.b(), ns.e('placeholder')]}>
                                ...
                                <ul class={[formControlNs.b(), ns.e('files-list')]}>
                                    {fileUploaders.value.map((fileUploader, index) => (
                                        <li
                                            ...
                                            onClick={(event: Event) =>
                                                clickSelectedFile(event, fileUploader.file)
                                            }
                                        >
                      <span
                          class={[
                      ...
                      >
                        {fileUploader.file.name}
                                            ...
                                            {fileUploader.status === UploadStatus.failed && (
                                                <DIcon name='running' onClick={fileUpload}/>
                                            )}
                                            {fileUploader.status === UploadStatus.uploaded && (
                                                <DIcon name='right' color='#50d4ab'/>
                                            )}
                                        </li>
                                    ))}
                                </ul>
                 ...
                 <DIcon name='more-operate' title='12222' color='#252b3a1'/>
              </span>
                        </div>
                    )}
                </div>
