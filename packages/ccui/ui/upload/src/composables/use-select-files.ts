...
export const useSelectFiles = (
  ...
  const _validateFiles = (
    ...
    if (
      uploadOptions &&
      beyondMaximalSize(file.size, uploadOptions.maximumSize)
    ) {
      return {
        checkError: true,
        errorMsg: t('getBeyondMaximalFileSizeMsg')(
          file.name,
          uploadOptions.maximumSize || 0
        )
      };
    }
  ...
  const checkAllFilesSize = (fileSize: number, maximumSize: number) => {
    if (beyondMaximalSize(fileSize, maximumSize)) {
      BEYOND_MAXIMAL_FILE_SIZE_MSG.value = t(
