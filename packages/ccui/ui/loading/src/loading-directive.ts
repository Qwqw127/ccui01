...
import {
  createComponent,
  unmountComponent
} from '../../shared/scripts/component';

const loadingConstructor = Loading;
    ...
    case 'array':
      if (
        (value as Array<unknown>).some(
          (val: unknown) => getType(val) !== 'promise'
        )
      ) {
        console.error(
          new TypeError('Binding values should all be of type Promise')
        );
        return 'error';
      }
      return value;
...
const handleProps = (
  ...
  vprops: LoadingOptions | VNode['props'] | null
) => {
  const props = {
    ...new LoadingOptions(),
    ...vprops
...
const LoadingDirective = {
  ...
  updated: function (
    ...
    if (
      (!isEmpty(binding.value) && cacheInstance.has(el)) ||
      (isEmpty(binding.value) && !cacheInstance.has(el))
    ) {
      return;
    }
